...
module.exports = function (archive) {
  ...
  function downloadStatsMultiwriter () {
    var contentFeedsCounted = {}
    count.downloaded = 0
    setInterval(checkFeeds, 1000) // FIXME: Don't poll
    checkFeeds()

    function checkFeeds () {
      archive.db.contentFeeds.forEach(function (feed) {
        if (!feed || !feed.key) return
        var key = feed.key.toString('hex')
        if (!contentFeedsCounted[key]) {
          contentFeedsCounted[key] = bitfield()
        }
        for (var i = 0; i < feed.length; i++) {
          if (feed.has(i) && counted.set(i, true)) count.downloaded++
        }
